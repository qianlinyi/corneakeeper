...
from corneakeeper.models import User, Photo, Notification
from corneakeeper.notifications import push_collect_photo_notification, push_follow_notification
...
def get_profile(user_id):
    ...
    return render_template('main/profile_popup_{}.html'.format(request.cookies.get('language', 'cn')), user=user)
...
def collect(photo_id):
    ...
    if current_user != photo.author and photo.author.receive_collect_notification:
        push_collect_photo_notification(collector=current_user, photo_id=photo_id, receiver=photo.author)
