...
from corneakeeper.models import Notification
...
def push_collect_photo_notification(collector, photo_id, receiver):
...
def push_collect_post_notification(collector, post_id, receiver):
    message = 'User <a href="%s">%s</a> collected your <a href="%s">post</a>' % \
              (url_for('user.show_charts', username=collector.username),
               collector.username,
               url_for('blog.show_post', post_id=post_id))
    notification = Notification(message=message, receiver=receiver)
    db.session.add(notification)
    db.session.commit()
